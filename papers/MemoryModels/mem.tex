...
imagine a memory as a ``soup'' of locations, labelled by arbitrary
names, as illustrated in Figure~\ref{fig:soup}.

\begin{figure}
  \centering
  \begin{diagram}[width=150]
  drawLocation :: Diagram B -> Diagram B
  drawLocation lab = vsep 0.2 [roundedRect 1 1 0.1, lab]

  locs cow = map drawLocation
  ...
  \end{diagram}
  \caption{Memory as a soup of locations}
  \label{fig:soup}
\end{figure}
...
Along these lines, although we are using Agda as a concrete metalogic
in which to express our ideas, we make no particular commitments to
the specific metalogic used and what features it may or may not have
available.  For example, Agda has lots of built-in type formers (sums,
products, \emph{etc.}), but we will not take them for granted.
