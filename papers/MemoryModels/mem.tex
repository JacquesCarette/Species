...
If memory is a function mapping labels to values, then in
order to relabel we need a way to translate new labels to old, which
we precompose with the memory mapping itself.  That is, when given a
new label to look up, we first run it through the given function to
