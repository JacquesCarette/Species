...
\newif\ifcommentary\commentaryfalse
...
rewrite \AgdaFunction{Memory$_0$} and \AgdaFunction{lookup$_0$} as
follows:
%
\ExecuteMetaData[latex/models.tex]{naivememory2}

It is important to bear in mind that although we do use Agda as a
concrete metalogic, we make no particular commitments to the specific
metalogic used and what features it may or may not have available.
For example, Agda has many built-in type formers (sums, products,
\emph{etc.}) which allow one to construct new types out of existing
ones, but we will not take such type formers for granted.  In general,
the name of the game is to add structure in a gradual, carefully
controlled manner, so as to discover the minimum amount of structure
necessary to model various phenomena.
...
provide.  We will add more fields to this record as we discover more
structure we wish to impose on our notion of memory.
%
