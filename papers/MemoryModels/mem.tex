...
\item On the other hand, the relabelling function might not be
  ...
  is usually known as ``aliasing'', and tends to be viewed negatively.
  However, aliasing only has negative consequences in a context where
  mutable assignment is allowed!  When memory is read-only, it is
  impossible to observe whether two labels are aliased to a single
  value, or simply happen to map to two identical copies of the value.
  Hence in a read-only context, aliasing is simply an optimization.
